...
def evaluate_multiclass(model, loader, split_name, logger, criterion=None):
    ...
    report_dict['loss'] = avg_loss  # 将损失添加到报告字典
...
def train_single_multiclass_model(model_name):
    ...
    try:
        # 2. 创建记录CSV（新增）
        records_dir = os.path.join(CONFIG['output_dir'], 'training_records')
        os.makedirs(records_dir, exist_ok=True)
        record_csv_path = os.path.join(records_dir, f'{model_name}_epoch_losses.csv')
        
        with open(record_csv_path, 'w', newline='', encoding='utf-8') as f:
            csv_writer = csv.writer(f)
            csv_writer.writerow(['Epoch', 'Train_Loss', 'Val_Loss', 'Val_Macro_F1', 'Test_Loss', 'Test_Macro_F1'])
        ...
        for epoch in range(CONFIG['epochs']):
            ...
            # 测试（每个epoch都评估，带损失计算）
            test_report = evaluate_multiclass(model, test_loader, "Test", logger, criterion)
            test_loss = test_report['loss']
            test_macro_f1 = test_report['macro avg']['f1-score']
            
            # 记录到CSV（新增）
            with open(record_csv_path, 'a', newline='', encoding='utf-8') as f:
                csv_writer = csv.writer(f)
                csv_writer.writerow([
                    epoch+1, 
                    f"{train_loss:.4f}", 
            ...
            if val_macro_f1 > best_val_macro_f1:
                ...
                logger.info(f"✅ 保存最佳模型（Epoch {epoch+1} | 验证宏平均F1: {best_val_macro_f1:.4f} | Val Loss: {val_loss:.4f}）")
                print(f"✅ 保存最佳模型（Epoch {epoch+1} | 验证宏平均F1: {best_val_macro_f1:.4f} | Val Loss: {val_loss:.4f}）")
            
            # 学习率调度（基于验证损失）
            scheduler.step(val_loss)
            
            # 当前epoch总结（新增）
            logger.info(f"\n📊 Epoch {epoch+1} 总结 | Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f} | Test Loss: {test_loss:.4f}")
            print(f"📊 Epoch {epoch+1} 总结 | Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f} | Test Loss: {test_loss:.4f}")
        ...
        logger.info(f" - 对应测试损失：{checkpoint['best_test_loss']:.4f}")
        logger.info(f" - 最终测试损失：{final_test_report['loss']:.4f}")
        logger.info(f" - 最佳模型路径：{best_model_path}")
        logger.info(f" - 日志文件路径：{log_file}")
        logger.info(f" - 损失记录CSV：{record_csv_path}")
        logger.info(f"{'='*60}")
